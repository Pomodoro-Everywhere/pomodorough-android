...
def main() -> int:
    ...
    failures: list[str] = []
    checked = 0
    main_activity = (root / "app/src/main/java/me/egigoka/pomodorough/MainActivity.kt").read_text(
        encoding="utf-8"
    )
    expected_privacy_link = f'Uri.parse("{PRIVACY_POLICY_URL}")'
    if main_activity.count(expected_privacy_link) != 1:
        failures.append(
            "MainActivity.kt: privacy policy must use exact contractual URL "
